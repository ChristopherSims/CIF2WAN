...
class CIF2ESPRESSO:
    def __init__(self,input): # ciffile is the location of the CIF file
        ...
        self.frac_coords = crystal.get_scaled_positions(wrap=True)
 ...
 ################# ATOM PARAM ###############
            ...
            scf.write("\n\n") # formatting
            scf.write("ATOMIC_POSITIONS (crystal)\n")
            for index,x in enumerate(self.frac_coords):
                scf.write("%s %f %f %f\n" %(str(self.species[index]),x[0], x[1], x[2]))
            scf.write("\n\n")
            ###### CELL PARAMETERS ####
            #str(struct.lattice)
            scf.write("CELL_PARAMETERS (angstrom)\n")
            scf.write(str(self.lattice) + "\n")
            scf.write("\n\n")
            ###KPOINTS####
            scf.write("K_POINTS (automatic)\n")
            scf.write("%i %i %i 0 0 0\n" %(8, 8, 8))
...
if __name__ == "__main__":
    ...
    run = CIF2ESPRESSO(C2W)
    #print(parentfolder + "/CIFDIR/CIS.cif")
    run.generate()